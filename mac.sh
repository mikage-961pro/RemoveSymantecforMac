...
# File Name:    RemoveSymantecMacFiles.command
  Version=8.0.2
# Author:       Corey Swertfager, NortonLifeLock Inc.
# Created:      10/04/2001
# Modified:     04/03/2020
#
# WARNING: This script will remove all files and folders created by
#          Norton OS X products and any files within those folders.
...
#
# History: 8.0.0 - 09/06/2019 - Corey Swertfager:
#                  * Added removal prevention when running OS 10.15 or later
#                    via ProcessAppRemoval() function.
#          8.0.1 - 02/28/2020 - Corey Swertfager:
#                  * Updated ProcessAppRemoval() to also check for SEP app in
#                    "/Applications/Symantec Solutions" folder (MACSYS-8233).
#          8.0.2 - 04/03/2020 - Corey Swertfager:
#                  * Changed Author company to NortonLifeLock Inc.
#                  * Changed Symantec to Norton when writing to standard output.
...
'
   ...
   if $SymantecCrontabEntryExists ; then
      if $CreateFilesRemovedListOnly ; then
         if [ -z "$VolumeToDeleteCrontabsFrom" ] ; then
            echo "Norton crontab entries would be deleted from the current boot volume." >> "$FilesRemovedList"
         else
            echo "Norton crontab entries would be deleted from the volume" >> "$FilesRemovedList"
            echo "\"`basename "$VolumeToDeleteCrontabsFrom"`\"." >> "$FilesRemovedList"
         fi
         echo "" >> "$FilesRemovedList"
      else
         if [ -z "$VolumeToDeleteCrontabsFrom" ] ; then
            echo "Norton crontab entries were deleted from the current boot volume."
         else
            echo "Norton crontab entries were deleted from the volume"
      ...
      if $CreateFilesRemovedListOnly ; then
         if [ -z "$VolumeToDeleteCrontabsFrom" ] ; then
            echo "There are no Norton crontab entries on the current boot volume;" >> "$FilesRemovedList"
            echo "no crontab entries would be removed from it." >> "$FilesRemovedList"
         else
            echo "There are no Norton crontab entries on the volume \"`basename "$VolumeToDeleteCrontabsFrom"`\";" >> "$FilesRemovedList"
            echo "no crontabs would be adjusted on that volume." >> "$FilesRemovedList"
         fi
         echo "" >> "$FilesRemovedList"
      elif [ -z "$VolumeToDeleteCrontabsFrom" ] ; then
         echo "There are no Norton crontab entries to delete from the current boot volume."
      else
         echo "There are no Norton crontab entries to delete from the volume"
         echo "\"`basename "$VolumeToDeleteCrontabsFrom"`\"."
      fi
   fi
   return 0
...
'
   ...
   for EachHomeDir in $ComputerUsersHomeDirsAndRootDir ; do
      ...
      if $CreateFilesRemovedListOnly ; then
         echo "Norton login items would be removed from:" >> "$FilesRemovedList"
         echo "   \"$OriginalPlistFile\"" >> "$FilesRemovedList"
         continue
      fi
      # Purge Norton login item(s)
...
$Line"
      ...
      if [ `grep -c '<dict>$' "$TempScrapFile"` != `grep -c '</dict>$' "$TempScrapFile"` ] ; then
         echo "ERROR: Could not remove Norton login items from:"
      ...
      # Else if syntax is to be checked and plist contains bad syntax
         ...
         echo "ERROR: Could not remove Norton login items (plutil conversion failed) from:"
         echo "       $OriginalPlistFile"
      else
         echo "Removing Norton login items from:"
         echo "   \"$OriginalPlistFile\""
   ...
   if [ -f /usr/libexec/PlistBuddy ] ; then
      for EachHomeDir in $ComputerUsersHomeDirsAndRootDir ; do
         ...
         while [ "$NameKeyValue" ] ; do
            # If this is a Norton login item that should be deleted
   ...
   if ! $SymantecLoginItemsWereFound ; then
      if $CreateFilesRemovedListOnly ; then
         echo "No Norton login items were found." >> "$FilesRemovedList"
      else
         echo "No Norton login items were found."
      fi
   fi
   $CreateFilesRemovedListOnly && echo "" >> "$FilesRemovedList"
   # If some Norton items were purged from files on boot volume, trigger restart
...
WARNING: This script will remove all files and folders created by Norton
         ...
         you have created.
"
   echo "1 - Remove all Norton files/folders."
   echo
...
FinishCleanup()
{
   ...
   if $DoShowOnlyFilesThatShouldHaveBeenUninstalled ; then
      ...
      if $RemoveInvisibleFilesOnly ; then
         echo "NOTE: Not all of the invisible Norton files were removed."
         ...
         if $RestartMayBeNeeded ; then
            echo
            echo "Some Norton product files have been removed from the boot volume."
         else
   ...
   if $RemoveInvisibleFilesOnly ; then
      if $NoFilesToRemove ; then
         echo "There were no invisible Norton files to be removed."
      else
   ...
   echo
   if $RestartMayBeNeeded ; then
      printf "Norton product files have been removed from the boot volume"
      ...
      echo "Norton product files have been removed from the above volume(s)."
   fi
   return 0
}
...
RemoveAllNortonFiles()
{
   ...
   if $CreateFilesRemovedListOnly ; then
      $DoShowOnlyFilesThatShouldHaveBeenUninstalled || echo "Finding visible Norton files on:   $1" >&2
   elif $ShowFilesAsRemoved ; then
      echo "Locating visible Norton files in:   $1"
   else
      echo "Removing visible Norton files from:   $1"
   fi
   cd "$1"
   if [ "`pwd`" = "/" ] ; then
      VolumePrefix=""
...
'
   ...
   if [ $CreateFilesRemovedListOnly = false -a "z$CurrentVolumeBeingUsed" = z/ ] ; then
      # Kill Norton processes and attempt to remove Symantec folder again in case Symantec
...
RemoveCrashReporterLogs()
{
   ...
   #
   # CrashLogGrepPattern will match visible or invisible (name begins with a period) Norton files
...
RemoveFilesFromLibraryAndUserDirectories()
{
   ...
   #                 unless it is "/" (boot volume).
   # Summary:   Removes all Norton files & folders from each user's
...
RemoveInvisibleFilesFromVolume()
{
   ...
   cd "$1"
   if $CreateFilesRemovedListOnly ; then
      $DoShowOnlyFilesThatShouldHaveBeenUninstalled || echo "Finding invisible Norton files on: $1" >&2
   elif $ShowFilesAsRemoved ; then
      echo "Locating invisible Norton files in: $1"
   else
      echo "Removing invisible Norton files from: $1"
   fi
   ...
   for EachFullPath in $ListOfPaths ; do
      ...
      if ! $TheFileWasRemoved ; then
         if ! $ErrorOccurred ; then
            # Create LogFile
            echo "Norton files/folders not removed:" >"$LogFile"
            chmod a=rw "$LogFile"
            ErrorOccurred=true
         fi
...
RemoveLoginKeychainPasswords()
{
   ...
   $DoShowOnlyFilesThatShouldHaveBeenUninstalled || echo "Looking for Norton login keychain items" 2>/dev/null
   IFS='
'
...
RunPredeleteScripts()
{
   ...
   IFS='
'
   echo "Looking for predelete scripts in Norton uninstaller's Receipts folder"
...
if [ $RemoveFromAllVolumes = true -a $CreateFilesRemovedListOnly = false -a $RemoveCrontabEntriesOnly = false -a $RemoveInvisibleFilesOnly = false -a $AutoRunScript = false -a $RunningFromWithinAppBundleOrSupportFolder = false ] ; then
   echo
   printf "Are you sure you want to remove Norton files from ALL mounted volumes (y/n)? "
   if `YesEntered` ; then
      echo
   else
      echo
...
if $CreateFilesRemovedListOnly ; then
   ...
   echo "Removing Norton files..."
   ! $RemoveInvisibleFiles && echo "Invisible Norton files will not be deleted."
fi
if $RemoveCrontabEntriesOnly ; then
   echo "Only crontab entries $WillTense be removed."
fi
! $RemoveCrontabEntries && echo "Norton crontab entries $WillTense not be removed."
...
if [ $BootVolumeWillBeSearched = true -a $CreateFilesRemovedListOnly = false ] ; then
   # If some Norton process or kext is in memory, touch restart file
   ...
   if [ -f "$SymantecCleanupRestartFile" ] ; then
      echo
      echo "NOTE: You should now restart the computer to get Norton processes"
